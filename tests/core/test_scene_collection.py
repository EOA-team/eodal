...
def test_scene_collection(get_s2_safe_l2a, get_polygons_2, get_bandstack):
    ...
    assert rcoll.scene_properties.acquisition_time == \
        rcoll_id.scene_properties.acquisition_time, \
    ...
    assert scoll.timestamps[-1] == str(s2_ds_2.scene_properties.acquisition_time), \
        'wrong timestamp'
    assert scoll.timestamps[0] == str(s2_ds_1.scene_properties.acquisition_time), \
        'wrong timestamp'
    ...
    assert isinstance(scoll_daterange_openend, SceneCollection), \
        'expected a SceneCollection'
    assert len(scoll_daterange_openend) == 1, \
        'expected only a single scene in collection'
    # slice by date range, open start of slice
    scoll_daterange_openstart = scoll[:date(2022, 1, 1)]
    assert isinstance(scoll_daterange_openstart, SceneCollection), \
        'expected a SceneCollection'
    assert len(scoll_daterange_openstart) == 1, \
        'expected only a single scene in collection'
    # slice outside of daterange covered by SceneCollection
    assert scoll[date(1900, 1, 1):date(1901, 12, 31)].empty, \
        'SceneCollection returned must be empty'
...
def test_scene_collection_to_xarray(get_scene_collection):
    ...
    for idx in range(len(scoll)):
        assert (xarr.values[idx, :, :, :] ==
                scoll[scoll.timestamps[idx]].get_values()).all(), 'wrong '
...
def test_scene_collection_time_series(get_scene_collection, generate_random_points,
    ...
    assert polygons_ts.iloc[1]['median'] == \
        scoll[1000]['B02'].reduce(
            by=polygons_ts.iloc[1].geometry, method='median')[0]['median'], \
